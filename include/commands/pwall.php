...
class pwall
{
	...
	public static function execute($params)
	{
		...
		{
			$ts_query->sendMessage(1, $params['executor']['clid'], ' > Nie podano wiadomości do wysłania. Wpisz [b]!pwall <wiadomość>[/b].');
			return;
		}
		...
		foreach($ts_data['clients'] as $client)
		{
			foreach(self::$command['ignored_groups'] as $group)
				if($powerBot->hasGroup($client['client_servergroups'], $group))
					continue 2;
			$ts_query->sendMessage(1, $client['clid'], $params['arguments'][1]);
			$count++;
		}
