...
class antypoke
{
	...
	public static function execute($params)
	{
		...
		if($powerBot->hasGroup($params['executor']['client_servergroups'], self::$command['group_id']))
		{
			...
			$ts_query->sendMessage(1, $params['executor']['clid'], ' > Ranga [b]AntyPoke[/b] została odebrana.');
		}
		else
		{
			$ts_query->serverGroupAddClient(self::$command['group_id'], $params['executor']['client_database_id']);
			$ts_query->sendMessage(1, $params['executor']['clid'], ' > Ranga [b]AntyPoke[/b] została nadana. Jeżeli chcesz ją usunąć, wpisz ponownie [b]!antypoke[/b].');
		}
